       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // By default the m_indices ImDrawIdx are 16-bits, you can change them to 32-bits in imconfig.h if your engine doesn't support 16-bits m_indices.
 ...
 A: A primer on labels and the ID Stack...
   ...
   - When working with trees, ID are used to preserve the open/close state of each tree node.
     Depending on your use cases you may want to use strings, m_indices or pointers as ID.
      ...
      e.g. when displaying a list of objects, using m_indices or pointers as ID will preserve the
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    //    You'll need to handle the 4-bytes m_indices to your renderer. For example, the OpenGL example code detect index size at compile-time by doing:
    //      glDrawElements(GL_TRIANGLES, (GLsizei)pcmd->ElemCount, sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, idx_buffer_offset);
    //    Your own engine or render API may use different parameters or function calls to specify index sizes. 2 and 4 bytes m_indices are generally supported by most API.
    // C) If for some reason you cannot use 4 bytes m_indices or don't want to, a workaround is to call BeginChild()/EndChild() before reaching the 64K limit to split your draw commands in multiple draw lists.
...
// Note that imgui doesn't know the semantic of each entry of io.KeysDown[]. Use your own m_indices/enums according to how your back-end/engine stored them into io.KeysDown[]!
