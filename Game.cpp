...
void Game::SetLight(Shader* shader)
{
	shader->use();
...
void Game::Render(const glm::mat4& projection, const glm::mat4& view)
{
	for (int i = 0; i < this->objs.size(); i++) {
		if (objs[i]->IsHide()) continue;
...
void Game::RenderTree(BVHierarchy::Node** tree, const glm::mat4& projection, const glm::mat4& view)
{
	BVHierarchy::Node* node = *tree;
	if (node == nullptr)
		return;
...
void Game::Traversal(BVHierarchy::Node* node, RigidBody* rb, std::vector<BVHierarchy::Node*>& res) {
	if (node == nullptr)
		return;
...
bool Game::CollectColliders(RigidBody* rb, std::vector<RigidBody*>& collisions)
{
...
void Game::FreeTree(BVHierarchy::Node* node)
{
	if (node == nullptr)
		return;
	//treeDepth = 0;
...
void Game::PerformCollision(RigidBody* rb1, RigidBody* rb2)
{
	if (rb1 == rb2) return;
	if (true)
	{
